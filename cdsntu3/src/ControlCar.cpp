...
Point ControlCar::getPoint(const Mat &src, Rect rect)
{
    ...
    for (int i = limit; i < IMG_H - 100; i++) {
        ...
        if (centerX > obs.x - 50 && centerX < obs.x)
            centerX -= 5;
        if (centerX > obs.width - 50 && centerX < obs.width + 50)
            centerX += 5;
...
void ControlCar::driveCar(const Mat &view, const Mat &view1, int flag, bool flag2, Rect obj)
{
    ...
    bool right = checkRight(dst4, Rect(280, 130, 40, 20));
    ...
    if (dem == 20 && flagRightTurn == true)
    {
        flagRightTurn = false;
        temp = false;
        dem = 0;
    } else if (dem == 20 && flagLeftTurn == true)
    {
