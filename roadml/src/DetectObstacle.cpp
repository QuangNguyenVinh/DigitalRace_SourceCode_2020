...
{
    obsPub = nodeObs.advertise<std_msgs::Int32MultiArray>(OBSTACLE_TOPIC,1);
...
Rect DetectObstacle::showObj(const Mat &depthImg, const Mat &rgbImg)
{
    ...
    if(obs != null)
    {
        ...
        //rectangle(rgb, rect, Scalar(0,0,255), 3);

    }
    //imshow("obstacle", rgb);
    array.data.push_back(rect.x);
    array.data.push_back(rect.y);
    array.data.push_back(rect.width);
    array.data.push_back(rect.height);
    obsPub.publish(array);
    return rect;
}